...
export default function ModuleGuard({ children, endpoint }: ModuleGuardProps) {
  ...
  }, [authLoading, moduleLoading]);

  // Loading state
  if (authLoading || moduleLoading || !accessChecked) {
    return (
  ...
  // Tidak punya akses — tampilkan langsung tanpa redirect
  if (accessChecked && !hasAccess(endpoint)) {
    return (
