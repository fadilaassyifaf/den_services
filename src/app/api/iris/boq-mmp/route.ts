...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/report/boq-mmp`, {
      method: 'POST',
    ...
    // BOQ MMP returns ZIP directly — stream it back to client
    const contentType = response.headers.get('Content-Type') ?? 'application/zip';
    const contentDisposition = response.headers.get('Content-Disposition') ?? 'attachment; filename="boq_mmp_result.zip"';
    ...
    console.error('[BOQ MMP] Proxy error:', detail);
