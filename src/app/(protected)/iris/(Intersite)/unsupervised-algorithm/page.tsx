...
import { UnsupervisedTask } from '@features/iris/sub-features/intersite/utils/unsupervised-task-manager';
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '80vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <p className="text-sm text-gray-600 leading-relaxed">
            The system analyzes site locations to automatically group candidate sites into optimal rings based on distance and connectivity limits. It identifies the best hub locations and detects{' '}
            <span className="font-semibold text-[#11499E]">SPOF (Single Point of Failure)</span> risks to ensure network reliability.
          </p>
          <div>
            <p className="text-xs font-bold text-[#11499E] mb-2 uppercase tracking-wide">Required Excel Sheets</p>
            <p className="text-xs text-gray-500 mb-3">Ensure your upload includes these specific sheets:</p>
            <div className="space-y-2">
              {[
                { sheet: 'sitelist', desc: 'Include site_id, site_name, site_type, lat, long, and region.' },
                { sheet: 'hubs', desc: 'Include hub site definitions and their connection points.' },
              ].map(({ sheet, desc }) => (
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => window.open('http://10.83.10.16:8000/template/Template_Unsupervised_Fiberization.xlsx', '_blank')}
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function UnsupervisedAlgorithmPage() {
  const { user, loading } = useAuth(true);
  const { addTask } = useProgress();

  const [excelFile, setExcelFile] = useState<File | null>(null);
  const [memberExpectation, setMemberExpectation] = useState('');
  const [memberExpectationError, setMemberExpectationError] = useState('');
  ...
  const [dropExistings, setDropExistings] = useState('false');
  ...
  const taskManagerRef = useRef<UnsupervisedTask | null>(null);

  useEffect(() => {
    taskManagerRef.current = new UnsupervisedTask();
  ...
  const handleExecute = async (e: React.FormEvent) => {
    ...
    if (!memberExpectation) { setMemberExpectationError('Member Expectation is required'); hasFieldError = true; }
    ...
    <>
      ...
      <div className="h-screen bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-3">
          <Sidebar activeMenu="Unsupervised Algorithm" />
          ...
          <div className="flex-1 border border-gray-200 bg-white relative flex flex-col" style={{ borderRadius: '14px' }}>
            ...
            <div className="bg-white px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0 flex items-start justify-between" style={{ borderRadius: '14px 14px 0 0' }}>
              <div>
                <h1 className="text-xl font-bold text-[#11499E] mb-0.5">Unsupervised Ring</h1>
                <p className="text-[#1E99D5] text-xs">Design fiber rings automatically using distance-based clustering algorithms.</p>
              </div>
              <button
                type="button"
              ...
              <form onSubmit={handleExecute}>
                  ...
                  >
                    ...
                    {excelFile ? (
                      ...
                      <>
                        ...
                        <p className="text-xs font-medium text-gray-600">(Excel file with sitelist and hubs sheets)</p>
                ...
                <div className="flex flex-col gap-2 mb-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {/* Member Expectation */}
                    <Field label="Member Expectation" hint="Member expectation in one ring" required>
                      <input
                        type="text" inputMode="numeric" pattern="\d*" value={memberExpectation}
                        onChange={(e) => handleIntegerInput(e.target.value, setMemberExpectation, setMemberExpectationError)}
                        placeholder="Input Value"
                        className={`w-full px-3 py-2 border rounded-lg text-xs outline-none focus:ring-2 focus:ring-[#11499E] text-[#11499E]
                          ${memberExpectationError ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
                      />
                      {memberExpectationError && (
                    ...
                    {/* Drop Existings */}
                    <Field label="Drop Existings" hint="Drop ring if not containing new site">
                      <select
                        value={dropExistings} onChange={(e) => setDropExistings(e.target.value)}
                    ...
                    {/* Max Distance */}
                    <Field label="Max Distance" hint="Maximum distance to route (meters)" required>
                      <input
                  ...
                  {/* Error / Success messages - OUTSIDE grid, full width */}
                  {error && (
                  ...
                  {/* Execute Button - OUTSIDE grid, aligned right */}
