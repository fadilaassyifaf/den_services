...
import { DrmIntersiteTask } from '@features/iris/sub-features/report/utils/drm-intersite-task-manager';
...
export default function DrmIntersitePage() {
  ...
  const taskManagerRef = useRef<DrmIntersiteTask | null>(null);

  useEffect(() => {
    taskManagerRef.current = new DrmIntersiteTask();
      ...
      <div className="flex flex-1 overflow-hidden gap-3 p-3">
        ...
        <div className="flex-1 overflow-y-auto border border-gray-200 bg-white relative" style={{ borderRadius: '14px' }}>
          <div className="sticky top-0 z-10 bg-white px-5 pt-5 pb-4 border-b border-gray-100">
            ...
            <p className="text-[#1E99D5] text-xs m-0">Create DRM Report based on Design KMZ</p>
          </div>
            ...
            <form onSubmit={handleExecute}>
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-[#11499E] transition ${designFile ? 'border-[#11499E] bg-blue-50' : 'border-gray-300'}`}
                >
                  ...
                  <p className="text-xs font-medium text-gray-700 text-center">
                    {designFile ? <span className="text-[#11499E] font-semibold">{designFile.name}</span> : 'Upload design file'}
                  </p>
                  <p className="text-[10px] text-gray-400">(.kmz, .kml — DEN intersite format)</p>
                  {designFile && <p className="text-[9px] text-gray-500 mt-1">{(designFile.size / 1024 / 1024).toFixed(2)} MB</p>}
                ...
                <div>
                  ...
                  <ul className="text-[10px] text-[#11499E] space-y-0.5 list-disc list-inside mb-3">
                    <li>KMZ file must contain <strong>['Connection', 'Route', 'FO Hub', 'Site List']</strong>.</li>
                    <li>Make sure the design format follows DEN intersite standard.</li>
                    <li>DRM report will be generated automatically based on design.</li>
                  </ul>
                  <p className="text-[10px] font-semibold text-[#11499E] mb-1">Input KMZ Design Sample</p>
                  <button
                    type="button"
              ...
              <div className="grid grid-cols-2 gap-3 mb-5">
                <div>
                  <label className="flex items-center gap-1 text-xs font-medium text-[#11499E] mb-1.5">
                    Operator
                    <span className="text-gray-400 text-[10px] cursor-help" title="Operator to generate design report based on">ⓘ</span>
                  </label>
                  <select
                    value={operator}
                    onChange={(e) => {
                ...
                <div>
                  <label className="flex items-center gap-1 text-xs font-medium text-[#11499E] mb-1.5">
                    Separator
                    <span className="text-gray-400 text-[10px] cursor-help" title="Separator for segment identify near end and far end">ⓘ</span>
                  </label>
                ...
                <div className="col-span-2">
                  <label className="flex items-center gap-1 text-xs font-medium text-[#11499E] mb-1.5">
                    Project Name
                    <span className="text-gray-400 text-[10px] cursor-help" title="Project name to write in DRM Report">ⓘ</span>
                  </label>
                  <input
                    type="text"
                    value={projectName}
