// src/features/iris/components/ModuleGuard.tsx
'use client';
...
export default function ModuleGuard({ children, moduleGroup }: ModuleGuardProps) {
  const router = useRouter();
  const { user, loading, hasAccess } = useAuthContext();

  const canAccess = !loading && !!user && hasAccess(moduleGroup);
  const isDenied  = !loading && !!user && !hasAccess(moduleGroup);
  ...
  useEffect(() => {
    if (isDenied) router.push('/unauthorized');
  }, [isDenied, router]);
  ...
  if (isDenied) {
    return <AccessDeniedScreen onBack={() => router.push('/home')} />;
  }
