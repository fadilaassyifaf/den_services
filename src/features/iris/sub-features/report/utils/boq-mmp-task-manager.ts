...
export interface TaskStatus {
  ...
  current_step?: string;
  message?: string;
  result?: { download_url?: string; output_file?: string };
}
...
export class BoqMmpTask {
  ...
  async submitTask(formData: FormData): Promise<{ success: boolean; task_id?: string; error?: string }> {
    try {
      this.cancelPolling();
      this.currentTaskId = null;
      this.abortController = new AbortController();

      const response = await fetch(`${this.API_BASE_URL}/boq-mmp`, {
        method: 'POST',
        body: formData,
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = parsed.detail || parsed.error || parsed.message || rawBody;
        } catch { errorMessage = rawBody; }
        throw new Error(errorMessage);
      }

      const data: TaskSubmitResponse = JSON.parse(rawBody);
      if (!data.task_id) throw new Error('No task_id received');
  ...
  private async pollTaskStatus() {
    ...
    try {
      const statusUrl = `${this.API_BASE_URL}/tasks/status/${taskId}`;
      const response = await fetch(statusUrl, { method: 'GET', headers: { Accept: 'application/json, application/zip' } });
      ...
      const rawBody = await response.text();
      if (!response.ok) throw new Error(`HTTP ${response.status}: ${rawBody}`);
      ...
      const progress = typeof raw.info?.progress === 'number' ? raw.info.progress :
        normalizedStatus === 'completed' ? 100 : normalizedStatus === 'pending' ? 0 : raw.progress ?? 0;
      ...
      if (normalizedStatus === 'completed' || normalizedStatus === 'failed') {
        this.cancelPolling();
        if (normalizedStatus === 'failed') this.handleError(infoMessage || 'Task failed');
        else this.notifyTaskCompleted(statusUrl);
      }
    } catch (error) {
      this.cancelPolling();
      this.currentTaskId = null;
      this.handleError(extractErrorMessage(error, 'Polling failed'));
    }
  }
  ...
  private async handleTaskComplete(response: Response, downloadUrl: string) {
    ...
    let filename = 'boq_mmp_result.zip';
  ...
  private notifyTaskCompleted(downloadUrl: string, filename?: string) {
    window.dispatchEvent(new CustomEvent('taskCompleted', {
      detail: { task_id: this.currentTaskId, download_url: downloadUrl, filename: filename || 'boq_mmp_result.zip' },
    }));
  }
  ...
  async downloadFile(url: string, filename?: string): Promise<boolean> {
    try {
      ...
      a.download = filename || 'boq_mmp_result.zip';
