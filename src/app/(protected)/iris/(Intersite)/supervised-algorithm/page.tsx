...
import { SupervisedTask } from '@features/iris/sub-features/intersite/utils/supervised-task-manager';
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '80vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <p className="text-sm text-gray-600 leading-relaxed">
            This tool builds optimized designs for rings you have already grouped. It validates site data, routes each ring independently based on your{' '}
            <span className="font-semibold text-[#11499E]">ring_name</span> assignments, and identifies{' '}
            <span className="font-semibold text-[#11499E]">SPOF</span> segments before generating the final KMZ output.
          </p>

          <div>
            <p className="text-xs font-bold text-[#11499E] mb-2 uppercase tracking-wide">Required Excel Columns</p>
            <p className="text-xs text-gray-500 mb-3">Ensure your upload includes these columns:</p>
            <div className="space-y-2">
              {[
                { sheet: 'site_id / site_name', desc: 'Unique site identifiers.' },
                { sheet: 'ring_name', desc: 'Your predefined cluster ID or ring assignment.' },
                { sheet: 'flag', desc: "Use '1' for the start site and '2' for the end site." },
                { sheet: 'Attributes', desc: 'Include site_type, lat, long, and region.' },
              ].map(({ sheet, desc }) => (
          ...
          <div>
            ...
            <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
              <li>Site type should contain <strong>'FO Hub'</strong> for interconnection source.</li>
              <li>Each ring name must be in the same region.</li>
          ...
          <div>
            ...
            <button
              type="button"
              onClick={() => window.open('http://10.83.10.16:8000/template/Template_Supervised_Fiberization.xlsx', '_blank')}
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function SupervisedAlgorithmPage() {
  ...
  const taskManagerRef = useRef<SupervisedTask | null>(null);

  useEffect(() => {
    taskManagerRef.current = new SupervisedTask();
    ...
    try {
      ...
      if (result?.success && result.task_id) {
        const newTask: ActiveTask = {
          ...
          task_name: 'Supervised Algorithm',
    ...
    <>
      ...
      <div className="h-screen bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-3">
          <Sidebar activeMenu="Supervised Algorithm" />
          ...
          <div className="flex-1 border border-gray-200 bg-white relative flex flex-col" style={{ borderRadius: '14px' }}>
            ...
            <div className="bg-white px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0 flex items-start justify-between" style={{ borderRadius: '14px 14px 0 0' }}>
              <div>
                <h1 className="text-xl font-bold text-[#11499E] mb-0.5">Supervised Algorithm</h1>
                <p className="text-[#1E99D5] text-xs">Create optimized ring designs based on your existing site clusters.</p>
              </div>
              <button
                type="button"
              ...
              <form onSubmit={handleExecute}>
                ...
                >
                  ...
                  {excelFile ? (
                    ...
                    <>
                      ...
                      <p className="text-xs font-medium text-gray-600">(Excel file with ring data)</p>
                ...
                <div className="flex flex-col gap-2 mb-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {/* Operator */}
                    <Field label="Operator" hint="Operator for intersite design project">
                      <select
                        value={operator}
                        onChange={(e) => {
