...
function extractFilename(path: string): string {
  if (!path) return '—';
  // Handle path unix maupun windows
      ...
      <div className="flex flex-1 overflow-hidden gap-4 p-4 max-w-[1600px] mx-auto w-full">
        ...
        <div className="w-56 flex-shrink-0 flex flex-col gap-3">
          ...
          <div className="p-4 bg-[#11499E] rounded-2xl shadow-sm relative overflow-hidden flex items-center justify-center" style={{ minHeight: 160 }}>
            ...
            <div className="relative z-10 flex flex-col items-center text-center gap-2 w-full">
              ...
              <div>
                <p className="text-white font-bold text-xs leading-tight">{user.name}</p>
                <p className="text-blue-200 text-[10px] mt-0.5">@{user.username}</p>
              </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: 'Total',      value: stats.total,      color: 'text-[#11499E]', bg: 'bg-white' },
              { label: 'Done',       value: stats.completed,  color: 'text-[#1E99D5]', bg: 'bg-white' },
              { label: 'Processing', value: stats.processing, color: 'text-amber-500', bg: 'bg-white' },
              { label: 'Failed',     value: stats.failed,     color: 'text-red-400',   bg: 'bg-white' },
            ].map(s => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex-1 flex flex-col">
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Filter Status</p>
            <div className="flex flex-col gap-1 flex-1">
              {['all', 'completed', 'processing', 'failed', 'pending'].map(s => (
                ...
                >
                  ...
                  {s !== 'all' && (
                    <span className={`text-[10px] font-bold tabular-nums ${filterStatus === s ? 'text-white/70' : 'text-gray-300'}`}>
        ...
        <div className="flex-1 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col border border-gray-200/60 min-w-0">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
            <div>
              <h1 className="text-[#11499E] font-bold text-base">Activity Log</h1>
              <p className="text-[#1E99D5] text-[11px] mt-0.5">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Search Tool */}
              <div className="relative">
                <input type="text" placeholder="Search by tool..."
                  value={searchService}
              ...
              <button onClick={fetchLogs} disabled={loading}
                className="p-1.5 rounded-xl border border-gray-200 hover:bg-gray-50 transition disabled:opacity-50" title="Refresh">
          ...
          {/* Column Headers — lebar fixed, tidak overflow */}
          <div className="flex-shrink-0 bg-gray-50/70 border-b border-gray-100">
            <div className="flex items-center px-5 py-2.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              <div className="w-[200px] flex-shrink-0">User</div>
              <div className="w-[160px] flex-shrink-0">Tool / Service</div>
              {/* File: flex-1 agar ambil sisa ruang, tapi tetap truncate */}
              ...
              <div className="w-[110px] flex-shrink-0">
                <button
                  ...
                  className="flex items-center gap-1 hover:text-[#11499E] transition-colors group"
                >
                  <span>Date</span>
                  <svg className={`w-3 h-3 transition-all ${sortOrder === 'asc' ? 'text-[#11499E] rotate-180' : 'text-gray-300'}`}
                    fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
              </div>
              <div className="w-[70px] flex-shrink-0 text-center">Result</div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="flex items-center px-5 py-3 border-b border-gray-50 animate-pulse gap-0">
                  ...
                  <div className="w-[160px] flex-shrink-0"><div className="h-2.5 bg-gray-100 rounded w-24" /></div>
                  <div className="flex-1 min-w-0"><div className="h-2.5 bg-gray-100 rounded w-40" /></div>
                  <div className="w-[110px] flex-shrink-0"><div className="h-5 bg-gray-100 rounded-full w-16" /></div>
                  <div className="w-[110px] flex-shrink-0"><div className="h-2.5 bg-gray-100 rounded w-20" /></div>
                  <div className="w-[70px] flex-shrink-0"><div className="h-6 bg-gray-100 rounded-lg w-10 mx-auto" /></div>
                </div>
              ))
            ) : paginated.length === 0 ? (
              ...
              paginated.map((log, idx) => {
                ...
                const filename = extractFilename(log.upload_file);
                return (
                  <div key={log.id}
                  ...
                  >
                    {/* User */}
                    <div className="w-[200px] flex-shrink-0 flex items-center gap-2.5 pr-3">
                    ...
                    {/* Tool */}
                    <div className="w-[160px] flex-shrink-0 pr-3 min-w-0">
                      <p className="text-xs font-semibold text-[#11499E] truncate">{log.service_name}</p>
                      <p className="text-[10px] text-gray-400 font-mono truncate">{log.service_router}</p>
                    </div>

                    {/* File — flex-1, truncate, tooltip untuk path lengkap */}
                    <div className="flex-1 min-w-0 pr-3">
                      <p
                        className="text-[11px] text-gray-600 font-mono truncate cursor-default"
                        title={log.upload_file}   // ← tooltip path lengkap saat hover
                      >
                        {filename}
                      </p>
                    </div>
                    ...
                    <div className="w-[70px] flex-shrink-0 flex justify-center">
                      {log.result_path && log.status === 'completed' ? (
                        <button
                          onClick={() => handleDownload(log.result_path!, filename)}
                          className="flex items-center gap-1 px-2 py-1 text-[10px] font-semibold text-[#1E99D5] bg-[#1E99D5]/5 hover:bg-[#1E99D5]/15 border border-[#1E99D5]/20 rounded-lg transition-colors"
                        >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-white flex-shrink-0">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}
                  className="px-2.5 py-1 text-[11px] font-semibold border border-gray-200 text-[#11499E] rounded-lg disabled:opacity-40 hover:bg-gray-50 transition">
                  Prev
                </button>
