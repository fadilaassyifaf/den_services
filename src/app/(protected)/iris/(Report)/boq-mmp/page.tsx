      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <p className="text-sm text-gray-600 leading-relaxed">
            Generate a <span className="font-semibold text-[#11499E]">Bill of Quantities (BOQ)</span> report
            optimized for <span className="font-semibold text-[#11499E]">Multi Access Point (MMP)</span> deployment.
            Uses tighter pole spacing and higher cable overhead compared to standard intersite BOQ.
          </p>
          ...
          <div>
            <p className="text-xs font-bold text-[#11499E] mb-2 uppercase tracking-wide">MMP vs Standard BOQ</p>
            <div className="overflow-hidden rounded-xl border border-gray-100">
              <table className="w-full text-[10px]">
                <thead>
                  <tr className="bg-[#11499E] text-white">
                    <th className="px-3 py-2 text-left font-semibold">Parameter</th>
                    <th className="px-3 py-2 text-center font-semibold">Standard</th>
                    <th className="px-3 py-2 text-center font-semibold">MMP</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ['Interval Pole', '80 m', '60 m'],
                    ['Cable Percentage', '10%', '15%'],
                    ['Cable Multiplier', '1×', '2×'],
                    ['Device in Site', 'OTB', 'ODP'],
                    ['Device in Branch', 'ODP', 'ODP'],
                  ].map(([param, std, mmp], i) => (
                    <tr key={param} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-3 py-2 text-gray-700 font-medium">{param}</td>
                      <td className="px-3 py-2 text-center text-gray-500">{std}</td>
                      <td className="px-3 py-2 text-center text-[#11499E] font-semibold">{mmp}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {[
                'Excel BOQ spreadsheet with MMP cost calculations',
                'Multi-access point equipment list',
                'Material requirements for MMP deployment',
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function BoqMmpPage() {
  ...
  const [intervalPoleM, setIntervalPoleM] = useState('60');
  const [cablePercentage, setCablePercentage] = useState('15');
  const [cableMultiplier, setCableMultiplier] = useState('2');
  const [deviceInSite, setDeviceInSite] = useState('ODP');
  ...
  const handleExecute = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/iris/boq-mmp', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
      ...
      const filename = parseFilename(
        contentDisposition,
        `BOQ_MMP_${iplFile.name.replace(/\.[^.]+$/, '')}.zip`
      );
      ...
      setSuccess(`BOQ MMP report generated: ${filename}`);
      setIplFile(null);
    ...
    <>
      ...
      <div className="h-screen bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-3">
          <Sidebar activeMenu="BoQ MMP Route" />
          ...
          <div className="flex-1 border border-gray-200 bg-white relative flex flex-col" style={{ borderRadius: '14px' }}>
            ...
            <div className="bg-white px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0 flex items-start justify-between" style={{ borderRadius: '14px 14px 0 0' }}>
              <div>
                <h1 className="text-xl font-bold text-[#11499E] mb-0.5">BOQ MMP</h1>
                <p className="text-[#1E99D5] text-xs">Generate BOQ report optimized for Multi Access Point (MMP) deployment.</p>
              </div>
              <button
                type="button"
