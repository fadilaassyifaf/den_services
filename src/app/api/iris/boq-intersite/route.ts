...
export async function POST(request: NextRequest) {
  try {
    ...
    // BOQ returns ZIP directly — stream it back to client
    const contentType = response.headers.get('Content-Type') ?? 'application/zip';
    const contentDisposition = response.headers.get('Content-Disposition') ?? 'attachment; filename="boq_intersite_result.zip"';
    ...
    console.error('[BOQ Intersite] Proxy error:', detail);
