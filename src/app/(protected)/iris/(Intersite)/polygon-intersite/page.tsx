...
import { PolygonIntersiteTask } from '@features/iris/sub-features/intersite/utils/polygon-intersite-task-manager';
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '80vh' }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <p className="text-sm text-gray-600 leading-relaxed">
            The system identifies candidate sites located within your defined service area and designs optimized
            rings restricted to those boundaries. It validates connectivity within the geographic constraints and
            detects <span className="font-semibold text-[#11499E]">SPOF</span> segments, making it ideal for
          ...
          <div>
            ...
            <div className="space-y-2">
              {[
                {
                  sheet: 'sitelist',
                  desc: 'Include site IDs, coordinates, and regions.',
                },
                {
                  sheet: 'hubs',
                  desc: 'Include hub definitions and primary connection points.',
                },
                {
                  sheet: 'Polygon File',
                  desc: 'Upload your service area boundaries. Supported formats: .kmz, .kml, .parquet, or .gpkg.',
                },
              ].map(({ sheet, desc }) => (
          ...
          <div>
            ...
            <ul className="text-xs text-gray-600 space-y-1 list-disc list-inside">
              ...
              <li>IOH operator separator will be <strong>'-'</strong> and XL operator separator will be <strong>';'</strong>.</li>
            </ul>
          </div>
...
// ── Main Page ──────────────────────────────────────────────────────────────────
export default function PolygonIntersitePage() {
  ...
  const polygonFileRef = useRef<HTMLInputElement>(null);
  const taskManagerRef = useRef<PolygonIntersiteTask | null>(null);

  useEffect(() => {
    taskManagerRef.current = new PolygonIntersiteTask();
    return () => { taskManagerRef.current?.cancelPolling(); };
  }, []);

  useEffect(() => {
    if (error || success) {
      const timer = setTimeout(() => { setError(''); setSuccess(''); }, 7000);
  ...
  const handleExecute = async (e: React.FormEvent) => {
    ...
    if (polygonFile && polygonFile.size > 100_000_000) { setError('Polygon file too large. Maximum size is 100MB'); return; }
    ...
    if (polygonFile) formData.append('polygon_file', polygonFile);
    ...
    try {
      ...
      if (result?.success && result.task_id) {
        ...
        if (polygonFileRef.current) polygonFileRef.current.value = '';
      } else {
    ...
    <>
      ...
      <div className="h-screen bg-white flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden gap-3 p-3">
          <Sidebar activeMenu="Polygon Intersite" />
          ...
          <div className="flex-1 border border-gray-200 bg-white relative flex flex-col" style={{ borderRadius: '14px' }}>
            ...
            <div className="bg-white px-5 pt-5 pb-3 border-b border-gray-100 flex-shrink-0 flex items-start justify-between" style={{ borderRadius: '14px 14px 0 0' }}>
              <div>
                <h1 className="text-xl font-bold text-[#11499E] mb-0.5">Polygon Intersite</h1>
                <p className="text-[#1E99D5] text-xs">Design fiber rings within specific geographic boundaries or polygons.</p>
              </div>
              <button
                type="button"
              ...
              <form onSubmit={handleExecute}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  ...
                  >
                    ...
                    {excelFile ? (
                      ...
                      <>
                        ...
                        <p className="text-xs font-medium text-gray-600">(Excel with sitelist and hubs sheets)</p>
                  ...
                  <div
                    ...
                    className={`border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-[#11499E] transition min-h-[160px]
                      ${polygonFile ? 'border-[#11499E] bg-blue-50' : 'border-gray-300 bg-white'}`}
                  >
                    <svg className="w-8 h-8 text-[#11499E] mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {polygonFile ? (
                      <>
                        <p className="text-xs font-semibold text-[#11499E] text-center px-4 truncate max-w-full">{polygonFile.name}</p>
                        <p className="text-[10px] text-gray-400 mt-0.5">{(polygonFile.size / 1024 / 1024).toFixed(2)} MB</p>
                      </>
                    ) : (
                      <>
